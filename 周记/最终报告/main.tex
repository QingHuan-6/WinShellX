...
本章按模块介绍 WinShellX 的设计与实现。各模块均给出流程图与运行效果截图，不再展开具体代码细节。流程图导出为 PNG 放入 \texttt{figures/}，控制台截图放入 \texttt{screenshots/}，重新编译后自动插入正文。
...
\begingroup
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.8cm} Y}
            ...
            内置命令 & 源文件 BuiltInCommands.cpp。实现 cd、dir、history、exit、tasklist、taskkill 等内部命令。 \\
...
程序总体执行流程与启动效果如图 \ref{fig:flow_main}、图 \ref{fig:startup} 所示。

\reportflowchart{flow_main_loop.png}{WinShellX 主程序总体流程图}{fig:flow_main}
\reportscreenshot{startup.png}{WinShellX 启动与命令提示符}{fig:startup}
...
用户每输入一条非空命令，模块都会将其追加到会话历史列表，并驱动状态在“等待输入—命令调度—等待输入”之间循环转移。若命令行末尾带有 \& 符号，则以外部命令后台方式启动子进程，主控模块不等待其结束即返回提示符。主控模块的状态转移与调度流程如图 \ref{fig:flow_shell} 所示，程序启动后的命令提示符界面如图 \ref{fig:startup} 所示。
...
当用户输入带引号的路径、带空格的多参数命令或空参数命令时，解析模块会尽量保持参数边界正确，避免把完整路径错误拆成多个 token。该模块的处理流程如图 \ref{fig:flow_parser} 所示；用户在控制台执行 help 命令后看到的命令列表，即注册表内容在界面上的具体呈现，如图 \ref{fig:help} 所示。
...
各内部命令彼此独立，新增命令时只需实现处理函数并在注册表中登记，无需修改主循环。图 \ref{fig:flow_builtin} 为内置命令模块的总体处理流程；图 \ref{fig:dir} 和图 \ref{fig:tasklist} 分别展示了 dir 与 tasklist 命令在控制台中的实际输出效果。
...
若在所有候选路径中均未找到匹配的可执行文件，模块输出“不是内部或外部命令”的错误提示，行为与 cmd 保持一致。对于需要接收管道输入或输出重定向的外部命令，模块还会创建匿名管道或绑定输出文件句柄，保证与左侧命令正确连接。图 \ref{fig:flow_external} 为外部命令执行流程；图 \ref{fig:external} 为执行 notepad 命令时的运行效果。
...
对于内部命令，模块通过临时替换标准输出流捕获文本；对于外部命令，则使用 CreatePipe、ReadFile 和 WriteFile 在进程间传递数据。需要说明的是，当前版本的后台模式与管道、重定向尚不支持同时使用，模块会在检测到冲突组合时给出明确提示。图 \ref{fig:flow_pipe} 为管道与重定向的处理流程；图 \ref{fig:redirect} 和图 \ref{fig:pipe} 分别为输出重定向与单级管道的运行效果。
...
历史记录的工作流程为：程序启动时从 .winshellx\_history 文件加载历史命令到内存；用户在提示符下每输入一条有效命令，模块即将其追加到历史向量尾部；执行 history 命令时按序号输出全部记录；程序退出时对历史去重，保留最近 200 条不重复命令并写回文件。别名的工作流程为：用户通过 alias name=command 创建映射关系，通过 unalias name 删除映射；别名保存在 .winshellx\_aliases 文件中，程序启动时加载，每次用户输入命令前由主控模块对首 token 做别名展开。

两个子功能协同工作时，用户可以先定义 alias ll=dir，之后输入 ll 等效于 dir；同时 history 命令能够显示包含 ll 和 dir 在内的全部历史记录。图 \ref{fig:flow_history} 为历史与别名模块的处理流程；图 \ref{fig:history} 和图 \ref{fig:alias} 分别为 history 命令与 alias 命令的运行效果。

\reportflowchart{flow_history_alias.png}{历史记录与别名模块流程图}{fig:flow_history}
...
智能补全采用三级优先策略：第一优先级为历史命令前缀匹配，第二优先级为内置命令名匹配，第三优先级为当前目录下的文件或文件夹路径匹配。只有当前一级未找到合适候选时，才继续尝试下一级。该策略保证用户重复执行最近命令时补全最快，同时也支持 dir s 这类路径补全场景。图 \ref{fig:flow_complete} 为行编辑与补全的处理流程；图 \ref{fig:tab} 为 Tab 补全的运行效果。
...
WinShellX 在实现上采用命令注册表统一管理内部命令，新增命令时无需改动主循环；将输入预处理与单命令执行分离，便于支持管道和重定向；历史、别名、补全和具体命令执行相互独立，便于分模块测试与维护；Win32 错误通过统一函数转换为可读提示，提升用户体验。
